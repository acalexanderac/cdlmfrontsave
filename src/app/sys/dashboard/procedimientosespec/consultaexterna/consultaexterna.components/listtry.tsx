...
const ConsultaexternaLists: React.FC = () => {
  ...
  const fetchPacientes = async (page: number) => {
       try {
        const response = await fetch(
            `${process.env.NEXT_PUBLIC_BACKEND_URL}/consultaexterna/sort?s=${searchTerm}&sort=${sortOrder}&page=${page}`,
            {
                method: 'GET',
                headers: {
            ...
            <div className="align-middle flex items-center flex-col pb-5">
                <table className="py-10 rounded-full pt-5 pb-10 w-1/2 ">
                    ...
                    <tr className='rounded-full pb-5'>
                        ...
                        <th className="p-3 text-sm font-semibold tracking-wide text-left">Fecha Consulta Externa </th>
                    ...
                    {patients.map((paciente: Treatment) => (
                        <tr key={paciente.id}>
                            ...
                            <td className="p-1 whitespace-nowrap">
                                <div className="text-left text-black">{paciente['fechaConsultaexterna']}</div>
                            </td>
                            ...
                            <td className="p-1 whitespace-nowrap">
                                <div className='pr-5 pl-5'>
                                    <Link href={`/sys/dashboard/procedimientosespec/consultaexterna/${paciente['id']}`}>
                                        <button
...
export default ConsultaexternaLists;
