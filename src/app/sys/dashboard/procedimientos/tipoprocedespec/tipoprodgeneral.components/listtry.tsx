  ...
  const fetchTipoT = async (page: number) => {
       try {
        const response = await fetch(
            `${process.env.NEXT_PUBLIC_BACKEND_URL}/tipotratamientoespec/sort?s=${searchTerm}&sort=${sortOrder}&page=${page}`,
            {
                method: 'GET',
                headers: {
                    ...
                    <tr key={tipoprodc['id']}>
                        ...
                        <td className="p-1 whitespace-nowrap">
                            <div className='pr-5 pl-5'>
                                <Link href={`/sys/dashboard/procedimientos/tipoprocedespec/${tipoprodc['id']}`}>
                                    <button
