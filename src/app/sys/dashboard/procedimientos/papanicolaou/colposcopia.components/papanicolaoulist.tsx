...
const PapanicolaouList: React.FC = () => {
    ...
    const fetchPacientes = async (page: number) => {
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_BACKEND_URL}/papanicolaous/sort?s=${searchTerm}&sort=${sortOrder}&page=${page}`,
                {
                    method: 'GET',
                    headers: {
                ...
                <table className="py-10 rounded-full pt-5 pb-10 w-1/2 ">
                    ...
                    {patients.map((paciente: Treatment) => (
                        <tr key={paciente.id}>
                            ...
                            <td className="p-1 whitespace-nowrap">
                                <div className="text-left text-black">{paciente['fechaPapanicolaous']}</div>
                            </td>
...
export default PapanicolaouList;
