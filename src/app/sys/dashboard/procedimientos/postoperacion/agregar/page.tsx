...
function PostoperacionFormPage() {
        ...
        if (params.id) {
            try {
                ...
                setNewProcedimiento((prevState) => ({
                    ...prevState || {},
                    fechaPostop: String(dataUpdate.fechaPostop || ''),
                    anotacionesCirugia: String(dataUpdate.anotacionesCirugia || ''),
                    observaciones: String(dataUpdate.observaciones || ''),
                    paciente: pacienteName || '', // Ensure it's a string
                    dpi: String(dataUpdate.dpi || ''),
                    tipoCirugia: String(dataUpdate.tipoCirugia || ''),
                }));
    ...
    const handleDelete = async () => {
        if (window.confirm('Are you sure you want to delete?')) {
            try {
                ...
                router.push('/sys/dashboard/procedimientos/postoperacion');
    ...
    const updateTask = async (data: FormData) => {
        try {
            ...
            const updateData = {
                fechaPostop: data.fechaPostop,
                anotacionesCirugia: data.anotacionesCirugia,
  ...
  <div className="flex flex-col gap-5 justify-center items-center">
          ...
          {!params.id ? 'Añadir Post Operación' : 'Editar Post Operación'}
        </span>
                </a>
<div className='justify-center items-center'>
                {params.id ? (
                    ...
                    >
                        Eliminar Post Operación ID. {params.id}
                    </button>
                ) : (
                    <button
                ...
                <div>
                    <form onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <div className="px-5 pt-5">
                                ...
                                <div className="relative mt-2 rounded-md shadow-sm">
                                    <input
                                        ...
                                        {...register('fechaPostop', { required: false })}
                                    />
                                </div>
                            </div>

                            <div className="px-5 pt-5">
                                <label htmlFor="tipoCirugia" className="block text-ls font-medium leading-6 text-gray-900">
                                    Tipo Cirugía
                                </label>
                                <div className="relative mt-2 rounded-md shadow-sm">
                                    <input
                        ...
                        rounded text-lg"
                                    type="submit"
                                >
                                    {!params.id ? 'Guardar Post Operación' : 'Modificar Post Operación'}
                                </button>
