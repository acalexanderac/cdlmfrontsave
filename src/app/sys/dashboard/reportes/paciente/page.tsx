...
useEffect(() => {
  ...
  const handleReporteTodosClick = () => {
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/pacientes/report`, {
      headers,
    })
  ...
  const handleReporteMenoresClick = () => {
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/pacientes/report/menores`, {
      headers,
    })
      .then(response => response.json())
      .then(data => {
  ...
  const handleReporteMayoresClick = () => {
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/pacientes/report/mayores`, {
      headers,
    })
      .then(response => response.json())
      .then(data => {
        ...
        <div className="lg:w-4/5 mx-auto flex flex-wrap">
          <div className="lg:w-1/2 w-full lg:pr-10 lg:py-6 mb-6 lg:mb-0">
            ...
            <div className="flex justify-between border-t border-gray-200 py-2">
              ...
              <button className="text-white bg-rose-900 border-0 py-2 px-5 focus:outline-none hover:bg-rose-300 rounded" onClick={handleReporteTodosClick}>
                Reporte Todos los Pacientes.
              </button>
            </div>
            <div className="flex justify-between border-t border-gray-200 py-2">
              <span className="text-gray-500 pr-5">Menores de Edad</span>
              <button className="text-white bg-rose-900 border-0 py-2 px-5 focus:outline-none hover:bg-rose-300 rounded" onClick={handleReporteMenoresClick}>
                Reporte Menores de Edad
              </button>
            </div>
            <div className="flex justify-between border-t border-gray-200 py-2">
              <span className="text-gray-500 pr-5">Mayores de Edad</span>
              <button className="text-white bg-rose-900 border-0 py-2 px-5 focus:outline-none hover:bg-rose-300 rounded" onClick={handleReporteMayoresClick}>
